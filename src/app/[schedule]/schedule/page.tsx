...
export default async function SchedulePage({
  ...
  const [roomsWithBookings, scheduleLayout, settings, openingHours, holds, { t }] = await Promise.all([
    getAllRoomsBookingsForDate(schedule.id, date),
    getScheduleLayout(schedule.id),
    getBookingSettings(schedule.id),
    getOpeningHoursForDate(schedule.id, date),
    getActiveHoldsForDate(schedule.id, date),
    getT(),
  ]);
  ...
  return (
    <div className="mx-auto w-full max-w-[1228px] flex-1 px-4 py-8">
      ...
      <div className="mt-6 flex items-center justify-between gap-2">
        <Link
          href={`/${slug}/schedule?date=${addDays(date, -1)}`}
        ...
        <DateNav date={date} today={todayStr()} basePath={`/${slug}/schedule`} />
        <Link
          href={`/${slug}/schedule?date=${addDays(date, 1)}`}
