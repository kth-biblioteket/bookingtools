...
export default async function RoomPage({
  ...
  const room = await getRoom(schedule.id, id);
  if (!room) notFound();
  ...
  const [bookings, holds, settings, openingHours, { t }] = await Promise.all([
    getBookingsForRoomInRange(schedule.id, id, weekStart, weekEnd),
    getActiveHoldsForRoomInRange(id, weekStart, weekEnd),
    getBookingSettings(schedule.id),
  ...
  return (
    <div className="mx-auto w-full max-w-4xl flex-1 px-4 py-8">
      <AutoRefresh />
      <Link href={`/${slug}/rooms`} className="text-sm text-kth-blue hover:underline">
      ...
      <div className="mt-6 flex items-center justify-between gap-2">
        <Link
          href={`/${slug}/rooms/${id}?date=${addDays(date, -7)}`}
        ...
        <DateNav date={date} today={todayStr()} basePath={`/${slug}/rooms/${id}`} />
        <Link
          href={`/${slug}/rooms/${id}?date=${addDays(date, 7)}`}
